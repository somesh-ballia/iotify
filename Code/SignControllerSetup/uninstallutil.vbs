...
'  uninstall the service , params - keep the window hidden (0) , don't wait (false)

If fso.FileExists(strFwkDir & "InstallUtil.exe") then  
'Remove Service First if found
If fso.FileExists(ServiceDir) then 
   ' shell.currentdirectory =  strFwkDir
	'shell.run chr(34) & strFwkDir & "InstallUtil.exe" & chr(34) & " /u " & chr(34) & ServiceDir & chr(34), 0, false
End If
	
End If

fso.DeleteFolder shell.ExpandEnvironmentStrings("%ProgramFiles%\KLA-Tencor\GenericEmailEngine"),True